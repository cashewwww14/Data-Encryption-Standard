...
from signature_lib import sign_message, generate_signature_keypair
...
def perform_key_exchange(client_socket):
    try:
        ...
        sig_public_key, sig_private_key = generate_signature_keypair(512)
        print("Signature keypair generated")
        
        # Kirim signature public key ke server
        e, n = sig_public_key
        sig_key_msg = {'type': 'signature_public_key', 'e': e, 'n': n}
        client_socket.send(json.dumps(sig_key_msg).encode('utf-8'))
        ...
        return master_key, sig_private_key
...
def bidirectional_chat(host, port, client_name):
    ...
    try:
        ...
        result = perform_key_exchange(client_socket)
        if not result:
            client_socket.close()
            return
        
        shared_key, sig_private_key = result
        ...
        while running:
            try:
                ...
                # Sign message
                signature = sign_message(plaintext, sig_private_key)
